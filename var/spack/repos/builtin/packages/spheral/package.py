...
class Spheral(CMakePackage, PythonPackage):
    ...
    depends_on('axom@0.5.0 +mpi +hdf5 -examples -python -fortran -umpire -raja', type='build')
    ...
    extends('python@2.7.18 +zlib +shared', type=['build', 'run'])

    depends_on('py-pip@9.0.1', type=['build'])
    depends_on('py-pybind11@2.4.3')
    depends_on('py-pyb11generator@1.0.12') # TODO: DNE
